...
<head>
    ...
    <title>Добавить сертификат - 7 Звезд</title>
    ...
    <main class="flex-grow">
        <div class="bg-gradient-to-r from-purple-100 to-white py-12">
            <div class="container mx-auto px-4 text-center">
                <h1 class="text-3xl font-bold gradient-text mb-2">Добавить сертификат</h1>
                <p class="text-lg text-gray-700">Загрузка нового сертификата или лицензии</p>
            </div>
        </div>
                        ...
                        <form action="{{ route('certificates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" 
                                        class="form-input w-full px-4 py-2 border border-gray-300 rounded-lg" required>
                                    <option value="license" {{ old('type') == 'license' ? 'selected' : '' }}>Лицензия</option>
                                    <option value="certificate" {{ old('type') == 'certificate' ? 'selected' : '' }}>Сертификат</option>
                                </select>
                            </div>
