...
<head>
    ...
    <title>Редактировать сертификат - 7 Звезд</title>
    ...
    <main class="flex-grow">
        <div class="bg-gradient-to-r from-purple-100 to-white py-12">
            <div class="container mx-auto px-4 text-center">
                <h1 class="text-3xl font-bold gradient-text mb-2">Редактировать сертификат</h1>
                <p class="text-lg text-gray-700">Изменение информации о сертификате</p>
            </div>
        </div>
                        ...
                        <form action="{{ route('certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                <label for="title" class="block text-sm font-medium text-gray-700 mb-2">
                                    Название <span class="text-gray-500 text-xs">(до 200 символов)</span>
                                </label>
                                <input type="text" name="title" id="title" 
                                       class="form-input w-full px-4 py-2 border border-gray-300 rounded-lg" 
                                       value="{{ old('title', $certificate->title) }}" required maxlength="200">
                            </div>
                            ...
                            <div>
                                ...
                                <img src="{{ asset('storage/' . $certificate->image) }}" 
                                     ...
                                     class="max-w-full h-64 object-contain rounded-lg border border-gray-200">
                            </div>

                            <div>
                                <label for="image" class="block text-sm font-medium text-gray-700 mb-2">Новое изображение</label>
                                <input type="file" name="image" id="image" 
                                       class="form-input w-full px-4 py-2 border border-gray-300 rounded-lg">
                                <p class="mt-1 text-sm text-gray-500">Оставьте пустым, чтобы не изменять. Допустимые форматы: JPEG, PNG, JPG, GIF, SVG. Максимальный размер: 2MB</p>
                            </div>

                            <div>
                                <label for="type" class="block text-sm font-medium text-gray-700 mb-2">Тип документа</label>
                                <select name="type" id="type" 
                                        class="form-input w-full px-4 py-2 border border-gray-300 rounded-lg" required>
                                    <option value="license" {{ old('type', $certificate->type) == 'license' ? 'selected' : '' }}>Лицензия</option>
                                    <option value="certificate" {{ old('type', $certificate->type) == 'certificate' ? 'selected' : '' }}>Сертификат</option>
                                </select>
                            </div>
